...
建议 Cursor 版本 **≥ 3.4.17**，当前已在 **3.17.8** 上验证补丁不会造成cursor无法启动或无法使用。
...
下载 `cursor-zh-cn-pack-x.x.x.vsix` 文件在cursor进行安装。
...
5. 使用 **一键重启并清理 Cursor**，然后会自动重新打开（如未重新打开请手动打开）。
...
- 补丁依赖当前 Cursor 版本的 bundle 结构，cursor升级后需要更新规则
